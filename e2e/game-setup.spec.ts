...
test.describe('Game Setup', () => {
  ...
  test('should default to 2 players selected', async ({ page }) => {
    // The 2 Players button should be highlighted (amber bg)
    const twoPlayersBtn = page.getByRole('button', { name: '2 Players' });
    await expect(twoPlayersBtn).toHaveClass(/bg-amber-500/);
  ...
  test('should allow selecting 3 players', async ({ page }) => {
    ...
    // The 3 Players button should now be highlighted
    const threePlayersBtn = page.getByRole('button', { name: '3 Players' });
    await expect(threePlayersBtn).toHaveClass(/bg-amber-500/);
  ...
  test('should allow selecting 4 players', async ({ page }) => {
    ...
    // The 4 Players button should now be highlighted
    const fourPlayersBtn = page.getByRole('button', { name: '4 Players' });
    await expect(fourPlayersBtn).toHaveClass(/bg-amber-500/);
  ...
  test('should assign default tokens to players', async ({ page }) => {
    ...
    // Check that token buttons with ring class (selected) exist for each player section
    const playerSections = page.locator('.rounded-lg.border.border-stone-200.p-3');
  ...
  test('should start the game when clicking Start Game', async ({ page }) => {
    ...
    // Should now see the game board (center has SF and MONOPOLY as separate elements)
    await expect(page.locator('.col-span-9.row-span-9').getByText('SF')).toBeVisible({ timeout: 10000 });
